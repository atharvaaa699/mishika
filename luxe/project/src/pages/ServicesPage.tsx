      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <div
              key={index}
              className="service-card glass-card fade-in-zoom tilt-card group relative overflow-hidden rounded-lg"
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black via-black/70 to-transparent p-6 flex flex-col justify-end backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all duration-300">
                <div className="text-[#C6A45C] mb-2">{service.icon}</div>
                <h3 className="text-xl font-serif mb-2">{service.title}</h3>
                <p className="text-gray-300 text-sm mb-4">{service.description}</p>
                <Link
                  to="/contact"
                  className="text-[#C6A45C] hover:text-white transition inline-flex items-center magnetic-button"
                >
        ...
        <div className="mt-16 text-center">
          ...
          <Link
            to="/contact"
            className="inline-flex items-center px-8 py-3 bg-[#C6A45C] text-black rounded neon-glow magnetic-button"
          >
            Contact Concierge
          </Link>
        </div>
